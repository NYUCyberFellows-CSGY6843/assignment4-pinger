...
def ping(host, timeout=1):
    ...
    vars = vars.append({'min': round(response['rtt'].min(), 2), 'avg': round(response['rtt'].mean(), 2), 'max': round(response['rtt'].max(), 2), 'stddev': round(response['rtt'].std(),2)}, ignore_index=True).astype(float)
